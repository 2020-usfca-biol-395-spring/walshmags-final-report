...
#load map
sf_map <- get_map("San Francisco, California", zoom = 12)
...
```{r close-up-downtown, include = FALSE}
#its hard to see the size of the dots downtown so im gonna zoom in on that
#Find location
dt <- c(lon = -122.4145, lat = 37.7847)
dt_map <- get_map(location = dt, zoom = 14)
#plot diesel PM and Asthma
ggmap(dt_map) +
  geom_point(aes(x = Longitude,
                 y = Latitude,
             ...
             data = city_data) +
  scale_color_viridis_c() +
  ggtitle("Diesel PM Emissions and Asthma in Downtown San Francisco") +
  labs(x = "Longitude",
       y = "Latitude")
...
city_data <- filter(enviroscreen_data, `California County` == "San Francisco",
                    `Census Tract` != "6075017902")
#load map
cal_map <- get_map("San Francisco, California", zoom = 12)
...
```{r downtown-poverty, include = FALSE}
#its hard to see the size of the points downtown because they're so close
#together, so lets zoom in
dt <- c(lon = -122.4145, lat = 37.7847)
dt_map <- get_map(location = dt, zoom = 14)
#plot new map
ggmap(dt_map) +
  geom_point(aes(x = Longitude,
             ...
             data = city_data) +
  scale_color_viridis_c() +
  ggtitle("Poverty and Asthma in Downtown San Francisco") +
  labs(x = "Longitude",
       y = "Latitude")
```
